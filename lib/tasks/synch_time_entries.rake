namespace :synch do
	task :time_entries => :environment do
		...
		time_entries.each do |te|
			...
			if (user_relation = user_relations.find_by(source_id: te[:user])) and ((issue_relation = issue_relations.find_by(source_id: te[:issue_id])) or ((project_relation = project_relations.find_by(source_id: project_match_id)) and (project_relation.start_date.blank? or project_relation.start_date <= te[:spent_on].to_date)))
