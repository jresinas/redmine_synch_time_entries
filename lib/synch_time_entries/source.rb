...
module SynchTimeEntries
	class Source
		...
		# Obtiene las imputaciones imputadas de un proyecto con fecha entre start_date y end_date
		def self.get_project_time_entries(project_id = nil, start_date = nil, end_date = nil, offset = 0)
			...
			total = 1
			while (offset < total)
				res = redmine_request(get_endpoint('time_entries'), 'get', {:offset => offset, :project_id => project_id, :spent_on => "><#{start_date}|#{end_date}"})
